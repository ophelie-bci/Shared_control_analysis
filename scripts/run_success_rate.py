    ...
    for experiment in experiments:
        ...
        for monkey in monkeys:
            ...
            if success_rate_ai_on_all and success_rate_ai_off_all:
                ...
                print(
                    f"delta=ON-OFF: median={test['median_delta']:.3f}, "
                ...
                summary_success_rates.append({
                    "experiment": experiment,
                    "monkey": monkey,
                    "n_trials_on": total_correct_on + total_incorrect_on,
                    "n_correct_on": total_correct_on,
                    "n_trials_off": total_correct_off + total_incorrect_off,
                    "n_correct_off": total_correct_off,
                    "mean_on": mean_on,
                    "std_on": std_on,
                    "mean_off": mean_off,
                    "std_off": std_off,
                    "mean_delta_on_minus_off": test["mean_delta"],
                    "median_delta_on_minus_off": test["median_delta"],
                    "pooled_on": pooled_on,
                    "pooled_off": pooled_off,
                    "pooled_delta_on_minus_off": pooled_on - pooled_off,
                    "n_sessions": len(success_rate_ai_on_all),
                    "wilcoxon_n_total": test["n_total"],
                    "wilcoxon_n_used": test["n_used"],
                    "wilcoxon_zero_differences": test["n_zero"],
                    "wilcoxon_statistic": test.get("W", np.nan),
                    "wilcoxon_p_value": test.get("p", np.nan),
                })

    # Export manuscript tables generated by this analysis.
    root_dir = pathlib.Path(__file__).resolve().parent.parent
    tables_dir = root_dir / "tables"
    tables_dir.mkdir(exist_ok=True)

    if summary_success_rates:
        pd.DataFrame(summary_success_rates).to_csv(
            tables_dir / "success_rate_session_summary.csv",
            index=False,
        )

    if all_summary_points:
        pd.DataFrame(all_summary_points).to_csv(
            tables_dir / "success_rate_target_gain_summary.csv",
            index=False,
        )

    # Keep the legacy table hook, but write it inside the project instead of
    # creating a literal f{experiment}_summary.csv in the working directory.
    if summary_table.rows:
        summary_table.finalize(
            save_path=tables_dir / "legacy_success_rate_summary.csv",
            display=True,
        )
